...
const API_BASE_URL = import.meta.env.VITE_APPDIRECT_API_URL || 'https://marketplace.appdirect.com/api';
...
export const getApplications = async (): Promise<AppDirectApplication[]> => {
  try {
    const response = await appdirectApi.get('/v1/admin/marketplace/applications');
...
export const getApplication = async (id: string): Promise<AppDirectApplication | null> => {
  try {
    const response = await appdirectApi.get(`/v1/admin/marketplace/applications/${id}`);
...
export const getSubscription = async (id: string): Promise<AppDirectSubscription | null> => {
  try {
...
export const createSubscription = async (applicationId: string, plan: string, account: any): Promise<any> => {
  try {
...
export const cancelSubscription = async (id: string, reason: string): Promise<any> => {
  try {
    const response = await appdirectApi.delete(`/v1/admin/subscriptions/${id}`, {
      data: {
...
export const createUser = async (userData: Partial<AppDirectUser>): Promise<AppDirectUser | null> => {
  try {
