...
const API_BASE_URL =
  import.meta.env.VITE_NETSUITE_API_URL ||
...
netsuiteApi.interceptors.request.use((config) => {
  ...
  config.headers["Authorization"] =
    `OAuth oauth_signature="${encodeURIComponent(signature)}",oauth_version="1.0",oauth_nonce="${nonce}",oauth_signature_method="HMAC-SHA256",oauth_consumer_key="${CONSUMER_KEY}",oauth_token="${TOKEN_ID}",oauth_timestamp="${timestamp}",realm="${ACCOUNT_ID}"`;
...
export const getSubsidiary = async (
  id: string,
): Promise<NetSuiteSubsidiary | null> => {
  try {
...
export const syncNetSuiteData = async (): Promise<{
  success: boolean;
  message: string;
}> => {
  try {
    ...
    return {
      success: true,
      message: "NetSuite data sync completed successfully",
    };
  } catch (error) {
