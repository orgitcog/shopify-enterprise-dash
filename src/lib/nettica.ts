...
const API_BASE_URL =
  import.meta.env.VITE_NETTICA_API_URL || "https://api.nettica.com/v1";
...
export const createNetwork = async (
  network: Partial<NetticaNetwork>,
): Promise<NetticaNetwork | null> => {
  try {
...
export const getNode = async (
  networkId: string,
  nodeId: string,
): Promise<NetticaNode | null> => {
  try {
    const { data } = await netticaApi.get(
...
export const getConnections = async (
  networkId: string,
): Promise<NetticaConnection[]> => {
  try {
    const { data } = await netticaApi.get(`/networks/${networkId}/connections`);
    return data.connections;
  } catch (error) {
    console.error(
      `Error fetching connections for network ${networkId}:`,
      error,
    );
    return [];
  }
