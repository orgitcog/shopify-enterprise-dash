...
const API_BASE_URL =
  import.meta.env.VITE_POS_API_URL || "https://api.shopify.com/pos";
...
// Sync data
export const syncPOSData = async (): Promise<{
  success: boolean;
  message: string;
}> => {
  try {
...
export const getMockAnalytics = (): POSAnalytics => {
  return {
    ...
    salesByHour: [
      ...
      { hour: 18, amount: 3389.8, transactionCount: 89 },
    ],
    salesByPaymentMethod: [
      {
        method: "credit_card",
